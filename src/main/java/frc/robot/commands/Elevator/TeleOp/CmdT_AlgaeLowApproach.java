...
public class CmdT_AlgaeLowApproach extends Command {
  ...
  public boolean isFinished() {
    return (Math.abs(m_ElevatorSubsystem.getHeightLaserMeters() - AlgaeLowApproach.height) < 0.1) && (Math.abs(m_ElevatorSubsystem.GetElbowAngle() - AlgaeLowApproach.elbow) < 5) && (Math.abs(m_ElevatorSubsystem.GetWristAngleWorldCoordinates() - AlgaeLowApproach.wrist) < 5) ;
  }
}
