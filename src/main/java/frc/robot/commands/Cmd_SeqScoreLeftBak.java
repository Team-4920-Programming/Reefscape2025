...
public class Cmd_SeqScoreLeftBak extends SequentialCommandGroup {
  /** Creates a new Cmd_SeqRemoveAlgea. */

  public Cmd_SeqScoreLeftBak(CoralElevatorSubsystem Coral_SS, SwerveSubsystem Drive_SS, ReefSurveySubsystem Reef_SS) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    //addRequirements(Drive_SS, Coral_SS);
    addCommands(
      new CmdT_MoveToLevel(Coral_SS),
      new CmdA_DriveToReefPositionV3_Relative(Drive_SS,1).withTimeout(3),
