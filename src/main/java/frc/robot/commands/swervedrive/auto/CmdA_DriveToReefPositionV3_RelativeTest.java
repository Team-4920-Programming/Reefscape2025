...
public class CmdA_DriveToReefPositionV3_RelativeTest extends Command {
  ...
  double xtolerance = 0.04;
  double thetaTolerance = Units.degreesToRadians(1.75);
  double px = 2;
  double py = 2;
  double pt = 5;
  public CmdA_DriveToReefPositionV3_RelativeTest(SwerveSubsystem DriveSubsystem, int Position) {
  ...
  public void execute() {
    ...
    if (!atSetpoint(RotVel, thetaTolerance)){
      if (  RotVel > 0 ){
        RotVel = Math.max(RotVel, minSpeed);
      }
      else if (YVel < 0 ){
        RotVel = Math.min(RotVel, -minSpeed);
      }
    }
    ...
    DogLog.log("Auto/DriveToReefRelative/Check/XPIDAtSetpoint", atSetpoint(RobotDelta.dx, xtolerance));
    DogLog.log("Auto/DriveToReefRelative/Check/YPIDAtSetpoint", atSetpoint(RobotDelta.dy, ytolerance));
  ...
  public boolean isFinished() {
    return atSetpoint(RobotDelta.dx, xtolerance) && atSetpoint(RobotDelta.dy, ytolerance) && atSetpoint(RobotDelta.dtheta, thetaTolerance);
  }
